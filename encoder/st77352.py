...
#   @micropython.native
  def text( self, aPos, aString, aColor, aFont, aSize = 1, nowrap = False ) :
    '''Нарисуйте текст в заданной позиции. Если строка достигает конца экрана,
       она переносится в aPos[0] на следующей строке.
       aSize может быть целым числом, которое будет равномерно изменять размер
       шрифта по осям w, h или a, или любым другим параметрам, индексация которых
       может производиться с помощью [0] или [1]'''
